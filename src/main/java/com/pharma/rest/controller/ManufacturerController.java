...
public class ManufacturerController {
  ...
  public ResponseEntity<Drug> registerDrug(@RequestBody DrugPayload drugPayload) {
    try {
      Drug registeredDrug = manufacturerFacade.registerDrug(CreateDrug.builder().tagId(drugPayload.getTagId()).drugName(drugPayload.getDrugName()).mfgDate(drugPayload.getMfgDate()).build(), drugPayload.getCrps());
